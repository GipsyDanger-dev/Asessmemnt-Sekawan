...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <main className="workspace"><aside className="sidebar"><div className="brand"><span className="brand-mark">V</span><span>VEHICLE OPS</span></div><div className="user-card"><span className="avatar">{user.name.slice(0, 1)}</span><div><b>{user.name}</b><small>{user.role === 'admin' ? 'Admin Pool' : 'Approver'}</small></div><span className="user-status" /></div><NavLink className="create-trip" to="/bookings">+ Booking baru</NavLink><nav><p>OPERATIONS</p><NavLink to="/dashboard">Overview</NavLink>{user.role === 'admin' ? <><NavLink to="/bookings">Pemesanan</NavLink><NavLink to="/vehicles">Kendaraan</NavLink><NavLink to="/fleet-monitoring">Fleet monitoring</NavLink><NavLink to="/drivers">Driver</NavLink><NavLink to="/regions">Region</NavLink><NavLink to="/users">User & Approver</NavLink></> : <NavLink to="/approvals">Approval inbox</NavLink>}<p className="nav-space">MANAGEMENT</p>{user.role === 'admin' && <><NavLink to="/reports">Laporan</NavLink><NavLink to="/activity-logs">Activity log</NavLink></>}<NavLink to="/profile">Profil akun</NavLink></nav><button className="signout" onClick={() => { logout(); navigate('/login') }}>↪ Keluar workspace</button></aside><section className="main-stage"><header className="app-topbar"><div><span className="topbar-dot" /> Sistem operasi armada</div><div className="topbar-actions"><button aria-label="Pencarian">⌕</button><button aria-label="Notifikasi">◔</button><span>{user.role === 'admin' ? 'Admin access' : 'Approver access'}</span></div></header><section className="main-content">{children}</section></section></main>
}
